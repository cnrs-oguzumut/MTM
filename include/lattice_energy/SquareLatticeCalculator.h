...
public:
    SquareLatticeCalculator(double scale, double cutoff_radius = 2.5);
...
private:
    ...
    const double r_cutoff_sq;  // Added for efficiency (matching TriangularLatticeCalculator)
    std::vector<std::vector<double>> square_basis;  // Changed from array to vector for consistency

    // Private helper methods (same pattern as TriangularLatticeCalculator)
