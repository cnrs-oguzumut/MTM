...
bool checkSquareDomainViolation(
  ...
  for (const auto &element : elements) {
    ...
    if (detC <= 0.0) {
      std::cout << "Violation: Non-physical deformation (det(C) <= 0)"
                << std::endl;
      return true;
    }
    ...
    // Handle three cases: c12 > 0, c12 < 0, and c12 == 0 (including -0.0)
    bool condition1 = (c12 > 0 && c12 < min_val);           // Positive c12
    bool condition2 = (c12 < 0 && std::abs(c12) < min_val); // Negative c12
    bool condition3 =
        (std::abs(c12) <=
         std::numeric_limits<double>::epsilon()); // c12 is effectively zero

    // Violation if NONE of the conditions are satisfied
    bool violation_found = (!condition1 && !condition2 && !condition3);

    if (violation_found) {
      // std::cout << "Violation found:" << std::endl;
      // std::cout << std::fixed << std::setprecision(16) << "  c11: " << c11
      //           << ", c22: " << c22 << ", c12: " << c12 << std::endl;
      // std::cout << "  min(c11,c22): " << min_val << std::endl;
      // std::cout << "  Condition 1 (0 < c12 < min): "
      //           << (condition1 ? "PASS" : "FAIL") << std::endl;
      // std::cout << "  Condition 2 (c12 < 0 && |c12| < min): "
      //           << (condition2 ? "PASS" : "FAIL") << std::endl;
      // std::cout << "  Condition 3 (|c12| ~= 0): "
      //           << (condition3 ? "PASS" : "FAIL") << std::endl;
      return true;
    }
  }
...
ChangeMeasures computeChangeMeasures(
    ...
    const alglib::real_1d_array &reference_points, double lattice_constant,
    const std::vector<ElementTriangle2D> &elements, const UserData *userData,
    const std::vector<Point2D> &points, bool check_angles,
  ...
  for (int i = 0; i < n; ++i) {
    ...
    if (std::abs(reference_points[i]) > 1e-12) { // Avoid division by zero
      sum_relative_change +=
          (diff * diff) / (reference_points[i] * reference_points[i]);
    }
  ...
  if (check_angles && !elements.empty() && !points.empty()) {
    ...
    for (size_t idx = 0; idx < userData->active_elements.size(); idx++) {
      ...
      const ElementTriangle2D &element =
          elements[userData->active_elements[idx]];

      // Create a temporary copy of the element if we need to apply external
      // deformation
      ...
      //     std::cout << "Skipping element with fewer than 3 nodes." <<
      //     std::endl;
      ...
      // Get vertex coordinates with effective translations using the
      // appropriate element
      Eigen::Vector2d v1 =
          points[idx1].coord + element_to_use->getEffectiveTranslation(0);
      Eigen::Vector2d v2 =
          points[idx2].coord + element_to_use->getEffectiveTranslation(1);
      Eigen::Vector2d v3 =
          points[idx3].coord + element_to_use->getEffectiveTranslation(2);
      ...
      double angle1 =
          std::acos((-side3.dot(side1)) / (side3.norm() * side1.norm())) *
          180.0 / M_PI;
      double angle2 =
          std::acos((-side1.dot(side2)) / (side1.norm() * side2.norm())) *
          180.0 / M_PI;
      double angle3 =
          std::acos((-side2.dot(side3)) / (side2.norm() * side3.norm())) *
          180.0 / M_PI;
      ...
      // If the largest angle is over 110 degrees or smallest angle is under 25
      // degrees, consider the triangle distorted
      ...
      if (max_angle > 110.0 || min_angle < 25.0) {
        ...
        std::cout << "max_angle: " << max_angle << " min_angle: " << min_angle
                  << std::endl;
        break;
      }
  ...
  measures.displacement_exceeds_half_lattice =
      max_abs_diff > (0.5 * lattice_constant);
...
std::vector<int>
analyzeElementReduction(std::vector<ElementTriangle2D> &elements,
...
int compareM3Activation(const std::vector<int> &m3_before,
                        const std::vector<int> &m3_after) {

  // Make sure vectors have the same size
  if (m3_before.size() != m3_after.size()) {
    std::cerr << "Error: Vector size mismatch in compareM3Activation"
              << std::endl;
    return false;
  }
  ...
  if (diff_count > 0) {
    std::cout << "Found " << diff_count << " differences in M3 activation"
              << std::endl;
    // return true;
  }
