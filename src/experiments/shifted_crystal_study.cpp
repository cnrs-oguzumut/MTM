...
// ============================================================================
// shifted_upper_crystal_study
//
// Companion to single_dislocation_study (see dislocation_study.cpp). Same
// stripped-down Conti-Zanzotto pipeline (reference geometry, energy potential,
// lattice generation, domain maps, DOF mapping, meshing, strain-energy
// calculator), but instead of seeding a Volterra dislocation field it rigidly
// shifts the UPPER half of the crystal by ONE lattice spacing in x.
//
// The mesh is built on the pristine lattice first; the shift is then applied
// to the already-meshed atoms (every atom above the mid-plane gets x += a).
// A single L-BFGS relaxation follows (remeshing toggle kept, default off), and
// the initial and relaxed states are written to VTK with a defect analysis.
// ============================================================================
void shifted_upper_crystal_study(int caller_id, int nx, int ny) {
  ...
  // Mesh is generated BEFORE the shift, so the connectivity and reference
  // configuration come from the perfect crystal.
  ...
  // ==================== SHIFT UPPER HALF OF THE CRYSTAL ================
  // Rigidly translate every atom above the mid-plane by one lattice spacing in
  // x. Applied after meshing so the reference mesh stays the perfect crystal.
  double shift_amount = 0 * lattice_constant;
  // double y_mid = 0.5 * (domain_size.get_height()); // mid-plane
  double y_mid = 0.5 * (domain_size.get_height()) -
                 lattice_constant; // one plane below mid
  ...
  std::cout << "Shifted upper half (" << shifted_count
            << " atoms above y = " << y_mid << ") by " << shift_amount
            << " in x" << std::endl;

  // ==================== INSTALL A SINGLE DISLOCATION ====================
  // Seeded ON TOP OF the shifted crystal.
  Eigen::Vector2d burgers_vector(lattice_constant, 0.0); // unit Burgers, x-dir
  double core_radius = 6.0;
  double poisson_ratio = 0.3;
  size_t middle_atom_index = findMiddleAtom(square_points, true);

  square_points =
      createSingleDislocation(square_points, burgers_vector, middle_atom_index,
                              core_radius, poisson_ratio);
  std::cout << "Installed single edge dislocation at atom " << middle_atom_index
            << " (|b| = " << burgers_vector.norm() << ") on the shifted crystal"
            << std::endl;
  ...
  std::cout << "Saved initial shifted config " << file_id << std::endl;
  ...
  // shifted configuration energy before relaxation; this single-shot study
  ...
  std::cout << "Saved relaxed shifted config " << post_file_id << std::endl;
}
