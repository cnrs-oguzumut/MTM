...
#include "../include/optimization/LatticeOptimizer.h" //userdata
...
void example_1_atomistic_square() {
    ...
    //double optimal_lattice_parameter =
find_optimal_lattice_parameter(potential_func,lattice_type); double
optimal_lattice_parameter = 1.06585; double lattice_constant =
...
domain_size.get_height());
    ...
    if(pbc==1){
        ...
        std::vector<Point2D> square_points_periodic =
LatticeGenerator::create_periodic_copies( square_points, domain_dims, offsets);
        // Create Delaunay triangulation
        std::vector<Triangle> triangulation =
MeshGenerator::createTrianglesFromPoints( square_points_periodic);
...
MeshGenerator::select_unique_connected_triangles( points_used_in_triangulation,
        triangulation,
        original_domain_map,
...
create_dof_mapping_original(points_used_in_triangulation, 0.5*lattice_constant,
0);
    ...
    const std::vector<size_t>& active_elements =
        initialize_active_elements(elements, full_mapping,
points_used_in_triangulation.size());
...
std::endl;
        ...
        Eigen::Matrix2d F_ext = Eigen::Matrix2d::Identity() + alpha * a1 *
n1.transpose();
        ...
        deform_boundary_nodes(points_used_in_triangulation, full_mapping,
F_ext);

        // Map points to solver array
        map_points_to_solver_array(x, points_used_in_triangulation,
interior_mapping, n_vars);
        ...
        // Run optimization
        alglib::minlbfgsoptimize(state, minimize_energy_with_triangles, nullptr,
&userData);

        // Get results
        alglib::minlbfgsresults(state, x, rep);

        std::cout << "Optimization completed: iterations=" <<
rep.iterationscount
                  << ", termination type=" << rep.terminationtype << std::endl;

        // Update points with final optimized positions
        map_solver_array_to_points(x, points_used_in_triangulation,
interior_mapping, n_vars);
...
void debug_deformation_tests() {
  ...
  // 1. Simple Shear Test (F = [1 alpha; 0 1])
  {
    ...
    if (!outFile.is_open()) {
      std::cerr << "Error: Could not open simple_shear_F_debug.txt for writing!"
                << std::endl;
    } else {
      // Write header
      outFile << "# Simple Shear Energy Debug (using F matrix)" << std::endl;
      outFile << "# alpha, C11, C12, C22, C11_reduced, C12_reduced, "
                 "C22_reduced, raw_energy, energy_minus_zero"
              << std::endl;
      ...
      for (double alpha = 0.0; alpha <= 1.0; alpha += 0.01) {
        ...
        // Write to file
        outFile << std::fixed << std::setprecision(6) << alpha << ", "
                << C(0, 0) << ", " << C(0, 1) << ", " << C(1, 1) << ", "
                << C_reduced(0, 0) << ", " << C_reduced(0, 1) << ", "
                << C_reduced(1, 1) << ", " << raw_energy << ", " << energy
                << std::endl;
      }

      outFile.close();
      std::cout << "Debug data written to simple_shear_F_debug.txt"
                << std::endl;
    }
  }
  ...
  // 2. Volume Change Test (F = [lambda 0; 0 lambda])
  {
    ...
    if (!outFile.is_open()) {
      std::cerr
          << "Error: Could not open volume_change_F_debug.txt for writing!"
          << std::endl;
    } else {
      // Write header
      outFile << "# Volume Change Energy Debug (using F matrix)" << std::endl;
      outFile << "# lambda, volume_ratio, C11, C22, C11_reduced, C12_reduced, "
                 "C22_reduced, raw_energy, energy_minus_zero"
              << std::endl;

      // Calculate energy for different lambda values
      for (double lambda = 0.8; lambda <= 1.2; lambda += 0.01) {
        // Create deformation gradient for isotropic volume change: F = [lambda
        // 0; 0 lambda]
        Eigen::Matrix2d F;
        F << lambda, 0.0, 0.0, lambda;
        ...
        // Write to file
        outFile << std::fixed << std::setprecision(6) << lambda << ", "
                << volume_ratio << ", " << C(0, 0) << ", " << C(1, 1) << ", "
                << C_reduced(0, 0) << ", " << C_reduced(0, 1) << ", "
                << C_reduced(1, 1) << ", " << raw_energy << ", " << energy
                << std::endl;
      }
      ...
      outFile.close();
      std::cout << "Debug data written to volume_change_F_debug.txt"
                << std::endl;
    }
  }
  ...
  // 3. Uniaxial Strain Test (F = [lambda 0; 0 1])
  {
    ...
    if (!outFile.is_open()) {
      std::cerr
          << "Error: Could not open uniaxial_strain_F_debug.txt for writing!"
          << std::endl;
    } else {
      // Write header
      outFile << "# Uniaxial Strain Energy Debug (using F matrix)" << std::endl;
      outFile << "# lambda, C11, C22, C11_reduced, C12_reduced, C22_reduced, "
                 "raw_energy, energy_minus_zero"
              << std::endl;
      ...
      for (double lambda = 0.8; lambda <= 1.2; lambda += 0.01) {
        ...
        // Write to file
        outFile << std::fixed << std::setprecision(6) << lambda << ", "
                << C(0, 0) << ", " << C(1, 1) << ", " << C_reduced(0, 0) << ", "
                << C_reduced(0, 1) << ", " << C_reduced(1, 1) << ", "
                << raw_energy << ", " << energy << std::endl;
      }

      outFile.close();
      std::cout << "Debug data written to uniaxial_strain_F_debug.txt"
                << std::endl;
    }
  }
}
...
size_t findMiddleAtom(const std::vector<Point2D> &points,
  ...
  if (verbose) {
    ...
    std::cout << "Middle atom coordinates: (" << points[middle_index].coord.x()
...
void debug_deformation_tests_triangular() {
  ...
  // 1. Simple Shear Test (F = [1 alpha; 0 1])
  {
    ...
    if (!outFile.is_open()) {
      ...
      for (double alpha = 0.0; alpha <= 2.0; alpha += 0.01) {
        ...
        // double raw_energy = energy_functions::phi_func(C_reduced);
        double raw_energy =
            calculator.calculate_energy(C_reduced, potential_func, 0);
        double energy = raw_energy - zero;
  ...
  // 2. Volume Change Test (F = [lambda 0; 0 lambda])
  {
    ...
    if (!outFile.is_open()) {
      ...
      // Output the results
      std::cout << "Optimal lambda (minimum energy): " << std::fixed
                << std::setprecision(16) << optimal_lambda << std::endl;
      std::cout << "Minimum energy: " << std::fixed << std::setprecision(16)
                << min_energy << std::endl;
      std::cout << "Equilibrium lattice distance: " << std::fixed
                << std::setprecision(16) << optimal_lambda << std::endl;
...
void example_2_conti_zanzotto_triangular() {
  ...
  if (pbc) {
    ...
    F_ext << 1.0, 0.0, 0.0, 1.0;

    // Generate periodic copies
    std::vector<Point2D> square_points_periodic =
        LatticeGenerator::create_periodic_copies(square_points, domain_dims,
                                                 offsets, F_ext);
  ...
  // Select unique connected triangles
  std::vector<Triangle> unique_triangles =
      MeshGenerator::select_unique_connected_triangles(
  ...
  for (size_t i = 0; i < alpha_values.size(); i++) {
    ...
    if (i == 0) {
      ...
      for (size_t i = 0; i < square_points.size(); i++) {
        ...
        square_points[i].coord = F_ext * square_points[i].coord + 0 * noise;
      }
    }
    ...
    while (shouldRemesh) {
      ...
      std::vector<Point2D> new_square_points_periodic =
          LatticeGenerator::create_periodic_copies(square_points, domain_dims,
                                                   offsets, F_ext);

      // Create new triangulation
      triangulation =
          MeshGenerator::createTrianglesFromPoints(new_square_points_periodic);
      ...
      elements = MeshGenerator::createElementTri2D(
          unique_triangles, square_points, original_domain_map,
          translation_map);
      ...
      for (auto &element : elements) {
        ...
        double jac =
            element.calculate_shape_derivatives(x); // current positions
      }
      ...
      optimizer_remesh.optimize(x, minimize_energy_with_triangles,
                                &newUserData);
      map_solver_array_to_points(x, square_points, interior_mapping, n_vars);
      // recalculate
      ...
      ChangeMeasures result = computeChangeMeasures(
          x, original_x_remesh, lattice_constant, elements, &userData,
      ...
      if (!shouldRemesh) {
        // ConfigurationSaver::calculateEnergyAndStress(&userData, post_energy,
        // post_stress);
        ConfigurationSaver::calculateEnergyAndStress(&newUserData, post_energy,
                                                     stress_tensor);
        post_stress = stress_tensor(0, 1);
...
void memory(int caller_id, int nx, int ny, int restart_iteration) {
  ...
  std::cout << "RESTARTING SIMULATION FROM ITERATION " << restart_iteration << std::endl;
  ...
  std::cout << "\nLoading configuration from iteration " << restart_iteration << "..." << std::endl;
  ...
  // ==================== LOAD MESH ELEMENTS ====================
  std::cout << "\nLoading mesh elements from iteration " << restart_iteration << "..." << std::endl;
  ...
  std::cout << "  Shear stress σ_xy: " << loaded_stress_tensor(0, 1) << std::endl;
  ...
  std::cout << "  From α = " << alpha_min << " to α = " << alpha_max << std::endl;
  std::cout << "  Steps: " << num_alpha_points << " (step size: " << step_size << ")" << std::endl;
  ...
  for (size_t i = 0; i < alpha_values.size(); i++) {
    double alpha = alpha_values[i];
    std::cout << "\n=== Processing α = " << alpha << " (restart step " << (restart_iteration + i + 1) << ") ===" << std::endl;
    ...
    std::cout << "Iteration " << (restart_iteration + i + 1) << " completed successfully" << std::endl;
  }
...
void analyze_data_from_folder(int caller_id, int nx, int ny,int iter_start,int iter_end,int n_eig) {
  ...
  std::cout << "Found " << localized_modes.size() << " localized modes" << std::endl;

  // 3. Compute energy landscapes for each localized mode
  std::string energy_dir = output_dir + "/energy_iter_" + std::to_string(iteration);
...
for (int i = 0; i < std::min(20, (int)localized_modes.size()); i++) {
    ...
    // Compute and save
    assembler.compute_energy_landscape(
    free_dofs,
    eigenvec,
    &userData,
    mode,
    results.eigenvalues[mode],
    1,
    160,energy_dir);
